...
#ifndef DATASTRUCTURES_AND_ALGORITHMS_IN_CPP_EXTENDEDARRAY_H
#define DATASTRUCTURES_AND_ALGORITHMS_IN_CPP_EXTENDEDARRAY_H
...
void arrayList<T>::reSet(
    int theSize) { // Set list size to theSize and ensure that element array
...
template <class T>
void arrayList<T>::checkIndex(
    int theIndex) const { // Verify that theIndex is between 0 and listSize - 1.
...
template <class T>
T &arrayList<T>::get(
    int theIndex) const { // Return element whose index is theIndex.
...
template <class T>
void arrayList<T>::set(
    int theIndex,
    const T &newValue) { // Set element whose index is theIndex to newValue.
                         // Throw illegalIndex exception if no such element.
    checkIndex(theIndex);
...
int arrayList<T>::indexOf(const T &theElement)
    ...
    int theIndex =
        (int) (find(element, element + listSize, theElement) - element);
...
void arrayList<T>::erase(
    int theIndex) { // Delete the element whose index is theIndex.
                    // Throw illegalIndex exception if no such element.
    checkIndex(theIndex);
...
#endif // DATASTRUCTURES_AND_ALGORITHMS_IN_CPP_EXTENDEDARRAY_H
